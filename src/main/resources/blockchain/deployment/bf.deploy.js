...
const erc777 = buildContract(cfx,'../ERC777.json');
...
async function run() {
    ...
    if (updateBoomflow || updateCRCL) {
        console.log(`\nAdd Boomflow to CRCL Whitelisted, current nonce ${nonce}`)
        await Promise.all(
            crcl_assets.map(async (token, index) => {
                ...
                return addWhitelisted(token, boomflowAddr, JSBI.add(nonce, JSBI.BigInt(index)),'addWhitelisted');
            })
        )
        log();
        nonce = await waitNonce(JSBI.add(nonce, JSBI.BigInt(crcl_assets.length)), sender);
    ...
    if (updateCRCL) {
        ...
        for (let i = 0; i < crcl_assets.length; ++i) {
            wait_list.push(addWhitelisted(crcl_assets[i], dexAdmin, nonce, 'addWhitelisted'));
...
async function deployCRCL(token, nonce) {
    ...
    return crcl.constructor(`CRCL ${name}`/*name*/, `L-${name}`/*symbol*/, 18, erc777_addr, 259200, token.isCFX)
        .sendTransaction(txParams)
        .executed()
        .then((receipt) => {
            const retAddr = receipt.contractCreated;
